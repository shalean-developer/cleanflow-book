...
const BookingContent = () => {
  ...
  const getServiceSlug = () => {
    ...
    if (bookingData.serviceName) {
      return bookingData.serviceName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
    }
    return 'select';
  };
  const nextStep = () => {
    const slug = getServiceSlug();
    const stepRoutes = [`/booking/service/select`, `/booking/service/${slug}/property`, `/booking/service/${slug}/schedule`, `/booking/service/${slug}/cleaner`, `/booking/service/${slug}/review`];
    if (currentStep < 5) {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-8 mt-8 my-0 mx-auto max-w-7xl">
          ...
          {currentStep < 5 && <div className="hidden lg:block">
              <BookingSummary />
            </div>}
        </div>
      </div>
