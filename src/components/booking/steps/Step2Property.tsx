...
export const Step2Property = ({ onNext, onBack }: Step2PropertyProps) => {
  const { bookingData, updateBooking } = useBooking();
  const [extras, setExtras] = useState<any[]>([]);
  const [selectedExtras, setSelectedExtras] = useState<Set<string>>(
    new Set(bookingData.extras.map(e => e.id))
  );
  ...
  const handleExtraToggle = (extra: any) => {
    ...
    const extrasArray = extras
      .filter(e => newSelected.has(e.id))
      .map(e => ({ id: e.id, name: e.name, price: e.base_price }));
    
    updateBooking({ extras: extrasArray });
  };
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="space-y-6">
        <div>
          <Label htmlFor="houseDetails">House Details (Optional)</Label>
          <Textarea
            id="houseDetails"
            placeholder="Any special details about your property..."
            value={bookingData.houseDetails || ''}
            onChange={(e) => updateBooking({ houseDetails: e.target.value })}
            className="mt-2"
            rows={3}
          />
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <Button
                ...
                size="icon"
                onClick={() => updateCount('bedrooms', 1)}
                disabled={bookingData.bedrooms === 8}
              >
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <Button
                ...
                disabled={bookingData.bathrooms === 1}
              >
                <Minus className="w-4 h-4" />
              </Button>
              ...
              <Button
                ...
                size="icon"
                onClick={() => updateCount('bathrooms', 1)}
                disabled={bookingData.bathrooms === 6}
              >
                <Plus className="w-4 h-4" />
              </Button>
            </div>
